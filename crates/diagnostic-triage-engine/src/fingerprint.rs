...
pub fn fingerprint(input: &FindingFingerprintInput<'_>) -> Result<Fingerprint, FingerprintError> {
    ...
    let tool_name = canonical_text("tool name", &input.tool.name, 64)?;
    let rule_id = input
        .tool
        .rule_id
        .as_deref()
        .map(|value| canonical_text("rule id", value, 128))
        .transpose()?;
    let symbol = input
        .symbol
        .map(|value| canonical_text("symbol", value, 512))
        .transpose()?;
...
fn canonical_text(
    ...
    if value.chars().count() > max_chars {
        return Err(FingerprintError::FieldTooLarge { field });
    }
