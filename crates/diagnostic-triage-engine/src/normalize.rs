...
/// trimmed. The components are encoded as a versioned byte stream with an
/// explicit presence tag and a fixed-width length prefix for every field. This
/// keeps arbitrary Unicode and control characters, including U+001F, inside a
/// field instead of using them as delimiters.
...
/// # Errors
///
/// Returns [`NormalizationError::EmptyMessage`] when no non-whitespace text remains.
